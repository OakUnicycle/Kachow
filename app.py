...
def index():
    ...
    comparison_word = request.form.get('comparison_word')
    ...
    if db_result == None: # if not in database -> find info
        ...
        extra_args = [comparison_word.lower()] if comparison_word else None 
        scores = getting_scores(url, extra_args)
    ...
    return render_template('index.html', title = data['title'], source = data['source_domain'], main_text = data['summary'], related_articles = data['related_articles'], scores=scores, is_initial_load=False, comparison_word=comparison_word)
